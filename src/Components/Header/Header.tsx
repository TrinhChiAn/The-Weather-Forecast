...
function Header(props: any) {
  ...
  const getLocalDate = (timezone: any) => {
    const now = new Date();
    const localDate = new Date(now.getTime() + (timezone - 7 * 3600) * 1000);
    const year = localDate.getFullYear().toString().padStart(2, '0');
  ...
  useEffect(() => {
    const updateTime = () => {
      setCurrentTime(getLocalTime(props.timezone || 7 * 3600));
      setCurrenDate(getLocalDate(props.timezone || 7 * 3600));
    };
